...
check_transection_interval = int(inftransection['check_transection_interval'])
...
def checkTransectionAndSendREStoLAZ():
    try:
        threading.Timer(check_transection_interval,
                        checkTransectionAndSendREStoLAZ).start()

        mydb = dbClient[dbName]
        mycol = mydb[transectiontb]

        myquery = {"worker_sync_retries": max_retry, "send_to_WFM": False}
