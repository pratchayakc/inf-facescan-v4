...
username = str(os.environ['ACK_USER'])
password = str(os.environ['ACK_PASS'])

topic = parent_topic+"/face/+/Ack"
deviceAckQueue = infqueue['deviceack']

hostname = socket.gethostname()

client_id = groupId+'@@@'+hostname+"-ack"
LOG_PATH = inflog['path']

logger = logging.getLogger('Forwarder-ack')
logger.setLevel(logging.DEBUG)

fileFormat = logging.Formatter(
    '{"timestamp":"%(asctime)s", "name": "%(name)s", "level": "%(levelname)s", "message": "%(message)s"}')
fileHandler = logging.FileHandler(LOG_PATH+"/inf-forwarder-ack.log")
...
def on_message(client, userdata, message):
    try:
        ...
        routingKey = exchange+".face.*.Ack"
        issuccess = alicloudAMQP.amqpPublish(
            exchange, routingKey, msg, deviceAckQueue)
