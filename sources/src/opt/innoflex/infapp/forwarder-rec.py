...
username = str(os.environ['REC_USER'])
password = str(os.environ['REC_PASS'])

topic = parent_topic+"/face/+/Rec"
recQueue = infqueue['devicerec']

hostname = socket.gethostname()

client_id = groupId+'@@@'+hostname+"-rec"
LOG_PATH = inflog['path']

logger = logging.getLogger('Forwarder-rec')
logger.setLevel(logging.DEBUG)

fileFormat = logging.Formatter(
    '{"timestamp":"%(asctime)s", "name": "%(name)s", "level": "%(levelname)s", "message": "%(message)s"}')
fileHandler = logging.FileHandler(LOG_PATH+"/inf-forwarder-rec.log")
...
def on_message(client, userdata, message):
    try:
        ...
        logger.debug(msg)
        routingKey = exchange+".face.*.Rec"
        issuccess = alicloudAMQP.amqpPublish(
            exchange, routingKey, msg, recQueue)
